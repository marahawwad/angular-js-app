(function () {
    ...
    /**
     * Components --> spical kind of directive theat uses to simplified configuration that assume some default which best practices
     * 
     *  ** principle of components **
     * 1. only control their own view and data
     *    - never modify data or DOM outside their scope
     *    - modifying create side-effects that lead to chaos
     *    - always use isolate scope
     * 2. have well-defined public API - input & output from the components -
     *    - input: use '<' and '@' bindings only 
     *    - never change property of passed in objecy or array --> passing by reference
     *    - output: use '&' for component event callbacks
     *    - pass data to callback through param map {key:val}
     * 3. wll-defined lifecycle
     ...
     *    - each one would have a well-defined input & output
     *    - 2-way data binding is minimized as much as possible
     */
    // name using in html
    // scope always isolate with no ability to change it 
    // using $ctrl.prop -- to access the bindings

    /*.component('name', {
        templateUrl: 'file.html',
        controller: compController, // empty func provided automatically , palced on scpoe with $ctrl
    ...
    ShoppingListCompController.$inject = ['$scope', '$element'];
    function ShoppingListCompController($scope, $element) {
        ...
        // executed once controller get instantiated 
        $ctrl.$onInit = function () {
            totalItems = 0;
            console.log("$onInit");
        };

        // executed once the obj changes
        // whates what is change outside 
